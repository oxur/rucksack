...
mod tests {
    ...
    #[test]
    fn test_new_basic() {
        let vsn_db = VersionedDB::new(vec![1, 2, 3], "1.0.0".to_string());
    ...
    fn test_new_empty_bytes() {
        let vsn_db = VersionedDB::new(vec![], "0.1.0".to_string());
    ...
    fn test_new_large_bytes() {
        ...
        let vsn_db = VersionedDB::new(large_vec.clone(), "2.0.0".to_string());
    ...
    fn test_from_bytes() {
        ...
        let vsn_db = VersionedDB::from_bytes(bytes.clone());
    ...
    fn test_bytes_getter() {
        ...
        let vsn_db = VersionedDB::new(original.clone(), "1.0.0".to_string());
    ...
    fn test_hash_different_for_different_bytes() {
        let vsn_db1 = VersionedDB::new(vec![1, 2, 3], "1.0.0".to_string());
        let vsn_db2 = VersionedDB::new(vec![4, 5, 6], "1.0.0".to_string());
        assert_ne!(vsn_db1.hash(), vsn_db2.hash());
    }

    #[test]
    fn test_hash_empty_bytes() {
        let vsn_db = VersionedDB::new(vec![], "1.0.0".to_string());
    ...
    fn test_hash_known_value() {
        let vsn_db = VersionedDB::new(vec![2, 4, 16], "1.2.3".to_string());
        assert_eq!(vsn_db.hash(), 2233391132);
    }

    #[test]
    fn test_version_parsing() {
        let vsn_db = VersionedDB::new(vec![1], "3.14.159".to_string());
    ...
    fn test_version_with_prerelease() {
        let vsn_db = VersionedDB::new(vec![1], "2.0.0-alpha.1".to_string());
        let version = vsn_db.version();
    ...
    fn test_version_comparisons() {
        let vsn_db = VersionedDB::new(vec![1, 2], "1.5.0".to_string());
    ...
    fn test_serialise_basic() {
        let vsn_db = VersionedDB::new(vec![10, 20], "1.0.0".to_string());
    ...
    fn test_serialise_empty() {
        let vsn_db = VersionedDB::new(vec![], "0.0.1".to_string());
        let result = vsn_db.serialise();
        assert!(result.is_ok());
    }
    ...
    fn test_deserialise_basic() {
        let original = VersionedDB::new(vec![1, 2, 3], "1.2.3".to_string());
    ...
    fn test_deserialise_truncated_bytes() {
        let original = VersionedDB::new(vec![1, 2, 3], "1.0.0".to_string());
        let serialised = original.serialise().unwrap();
        ...
        assert!(
            result.is_err(),
            "Truncated data should fail to deserialise"
        );
        if let Err(e) = result {
    ...
    fn test_serialise_deserialise_roundtrip() {
        let original = VersionedDB::new(vec![5, 10, 15, 20], "2.1.0".to_string());
        let serialised = original.serialise().unwrap();
        let deserialised = VersionedDB::deserialise(serialised).unwrap();
    ...
    fn test_serialise_deserialise_large_data() {
        ...
        let original = VersionedDB::new(large_data, "3.0.0".to_string());
        let serialised = original.serialise().unwrap();
        let deserialised = VersionedDB::deserialise(serialised).unwrap();
    ...
    #[test]
    fn test_equality() {
        let db1 = VersionedDB::new(vec![1, 2, 3], "1.0.0".to_string());
        let db2 = VersionedDB::new(vec![1, 2, 3], "1.0.0".to_string());
        let db3 = VersionedDB::new(vec![1, 2, 3], "1.0.1".to_string());
        let db4 = VersionedDB::new(vec![1, 2, 4], "1.0.0".to_string());
